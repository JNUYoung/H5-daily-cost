<template>
  <div class="home">
    ...
    <div class="header">
      ...
      <div class="data-wrap">
        <span class="time" @click="monthToggle"
          >{{ currentTime }} <i class="iconfont sort-down"
        /></span>
    ...
    <!-- 添加账单按钮 -->
    <div class="add" @click="addToggle" ref="addBtn">
...
export default {
  ...
  setup() {
    ...
    const getBillList = async () => {
      const { data } = await axios.get(
        `/bill/list?date=${state.currentTime}&type_id=${
          state.currentSelect.id || "all"
    ...
    // 修改addBtn的背景颜色
    const changeBtnBgc = () => {
      // addBtn.value.style.backgroundColor = "rgba(0, 0, 0, 0.2)";
    };
  ...
  mounted() {
   this.$nextTick(() => {
     const bs = new BetterScroll(".content-wrap", {
      scrollY: true
     });
     bs.on("scrollStart", () => console.log("scrolling"));
     bs.on("scrollEnd", () => console.log("scroll end"));
   })
  },
};
</script>
